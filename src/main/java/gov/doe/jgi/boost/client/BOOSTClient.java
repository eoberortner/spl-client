...
public class BOOSTClient {
   ...
   * @param fivePrimeVectorOverlap ... 5-prime-vector overlap
	 ...
	 * @throws JSONException 
	 */
	public OutputFiles saveJobReport(
			final String jobUUID, final JSONObject jobReport) 
		...
		if(!Files.exists(jobOutputDir)) {
			...
			files.boostResponse = Paths.get(jobOutputDir.toString(), "response-" + jobUUID + ".json");
			// write the response to a file
			FileUtils.writeStringToFile(
					files.boostResponse.toFile(), jobReport.toString(4));
			...
			if(jobReport.has("response")) {
				...
				while(it.hasNext()) {
					...
					if(responseObject instanceof JSONObject) {
						...
						// input sequences
						files.inputFile = Paths.get(jobOutputDir.toString(), jobUUID + "-input-file.sbol.xml");
						FileUtils.writeStringToFile(
							files.inputFile.toFile(), jsonResponseObject.getString("original-sequences-text"));

						
						// output sequences
						files.outputFile = Paths.get(jobOutputDir.toString(), jobUUID + "-output-file.sbol.xml");
						FileUtils.writeStringToFile(
							files.outputFile.toFile(), jsonResponseObject.getString("modified-sequences-text"));
					}
				}
			}
			...
			if(jobReport.has("provenance")) {
				...
				files.provenanceFile = Paths.get(jobOutputDir.toString(), jobUUID + "-provenance.sbol.xml");
				FileUtils.writeStringToFile(
						files.provenanceFile.toFile(), jsonProvenance.getString("output-document"));
			}
